...
class TestInsertAndExpire(unittest.TestCase):
    ...
    def run_and_check_flights_table(self, end_empty, mock_kafkaconsumer):
        ...
        with main.engine.begin() as conn:
            ...
            flights_in_table = conn.execute(select([c for c in main.flights.c if c.name != 'changed'and c.name != 'added']))
        ...
        expired_flights_in_table = main.engine.execute(select([c for c in main.flights.c if c.name != 'changed' and c.name != 'added']))

        if end_empty:
            self.assertEqual(expired_flights_in_table.fetchall(), [])
        else:
            self.assertEqual(expired_flights_in_table.fetchall(), self.expected_flight_table_results)
